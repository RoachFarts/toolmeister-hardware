...
const CartItem = ({cart, item}:Props) => {
    ...
    const handleMinus = () => {
        if ((existingProduct?.quantity as number) > 1) {
            ...
            toast.success(`${item.title.substring(0, 12)} quantity decreased`)
        }else{
    ...
    <div className='w-full grid grid-cols-5 mb-4 border py-2'>
        <div className='flex col-span-5 md:col-span-2 items-center gap-4 ml-4'>
            ...
            <Link href={`/product/${item?.slug.current}`}>
                <Image src={urlFor(item?.image).url()} alt={item?.title} width={200} height={200} className='w-32 h-32 object-contain' />
            </Link>
        ...
        <div className='col-span-5 md:col-span-3 flex items-center justify-between py-4 md:py-0 lg:px-0'>
            ...
            <div className='w-1/3 flex items-center gap-6 text-lg'>
                <button onClick={handleMinus} className='w-6 h-6 bg-gray-100 text-sm flex items-center justify-center hover:bg-darkOrange/10 cursor-pointer border-[1px] border-gray-300 hover:border-darkOrange hoverEffect'><FaMinus /></button>
                <p className='text-sm font-semibold'>{item?.quantity}</p>
                <button onClick={()=>{dispatch(increaseQuantity(item._id))
                toast.success(`${item.title.substring(0, 20)} quantity increased`)
                }} className='w-6 h-6 bg-gray-100 text-sm flex items-center justify-center hover:bg-darkOrange/10 cursor-pointer border-[1px] border-gray-300 hover:border-darkOrange hoverEffect'><FaPlus /></button>
            </div>
